...
const typeConfig: Record<InsightType, { icon: React.ElementType; colors: string; iconColor: string }> = {
    positive: {
        icon: TrendingUp,
        colors: 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-800/30',
        iconColor: 'text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/50',
    },
    suggestion: {
        icon: Lightbulb,
        colors: 'bg-amber-50 dark:bg-amber-900/10 border-amber-100 dark:border-amber-800/30',
        iconColor: 'text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/50',
    },
    warning: {
        icon: AlertTriangle,
        colors: 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-800/30',
        iconColor: 'text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/50',
    },
    celebration: {
        icon: PartyPopper,
        colors: 'bg-violet-50 dark:bg-violet-900/10 border-violet-100 dark:border-violet-800/30',
...
export function AIInsightCard({ insight, onDismiss }: AIInsightCardProps) {
    ...
    return (
        <div className={`p-4 rounded-xl border ${config.colors} relative group transition-all hover:shadow-sm`}>
            {onDismiss && (
                <button
                    onClick={onDismiss}
            ...
            <div className="flex gap-3">
                <div className={`p-2 rounded-lg ${config.iconColor} flex-shrink-0 flex items-center justify-center`}>
                    <Icon className="w-4 h-4" />
                </div>
                <div className="flex-1 pr-6">
                    <h4 className="font-semibold text-slate-900 dark:text-slate-100 text-sm mb-1 font-heading">
                        {insight.title}
                    </h4>
